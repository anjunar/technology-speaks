...
class GenericOneToManyProvider[E] extends PredicateProvider[util.Set[? <: EntityInterface], E] {
  override def build(context : Context[util.Set[? <: EntityInterface], E]): Unit = {
    ...
    if (value != null && ! value.isEmpty)
      ...
      predicates.addOne(join.get("id").in(value.stream().map(entity => entity.id).toList))
  }
}
